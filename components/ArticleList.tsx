...
const ArticleList = ({ userStore }: Props) => {
  ...
  const {
    ...
    `${API_URL}/articles?storeId=${userStore.id}&page=${currentPage}&pageSize=${LIMIT}&search=${debouncedSearchTerm}`,
    fetcher
  );
  ...
  const {
    ...
    `${API_URL}/articles/count?storeId=${userStore.id}&search=${debouncedSearchTerm}`,
    fetcher
  );
