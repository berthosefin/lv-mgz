...
const ArticleList = ({ userStore }: Props) => {
  ...
  const {
    ...
    `/api/articles?storeId=${userStore.id}&page=${currentPage}&pageSize=${LIMIT}&search=${debouncedSearchTerm}`,
    fetcher
  );
  ...
  const {
    ...
    `/api/articles/count?storeId=${userStore.id}&search=${debouncedSearchTerm}`,
    fetcher
  );
